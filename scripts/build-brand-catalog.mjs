    ...
    walk(root, (node) => {
      if (node.type === "TEXT" && node.style) {
        ...
        if (node.style.fontSize) {
          fontSizeCounts.set(
            node.style.fontSize,
            (fontSizeCounts.get(node.style.fontSize) || 0) + 1
          );
        }
...
async function fetchDeepNodes(nodeIds, fetchDeep) {
  ...
  for (let i = 0; i < missing.length; i += 12) {
    ...
    process.stdout.write(
      `fetching deep nodes ${i + 1}-${Math.min(i + batch.length, missing.length)}... `
    );
...
function inferStudioReadiness(category) {
  ...
  if (
    ["seven-day-book", "matters-town-quotes", "matters-lab-social", "freewrite-core"].includes(
      category.id
    )
  ) {
    return "high";
...
function renderNewcomerGuide(catalog) {
  ...
  const sizeRows = catalog.categories
    ...
    .map(
      (category) =>
        `| ${category.label} | ${category.commonOutputSizes
          .slice(0, 5)
          .map((item) => `${item.size} x${item.count}`)
          .join("<br>")} |`
    );
  ...
  for (const useCase of STUDIO_USE_CASES) {
    lines.push(
      `- ${useCase.label}：欄位 ${useCase.requiredInputs.join(", ")}；輸出 ${useCase.recommendedOutputs.join(", ")}。`
    );
  }
  lines.push("");
