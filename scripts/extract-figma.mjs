...
async function crossCheck() {
  ...
  for (const [type, list] of Object.entries(byType)) {
    ...
    for (const s of list.sort((a, b) => a.name.localeCompare(b.name))) {
      lines.push(
        `| ${s.name} | \`${s.node_id}\` | ${s.description?.replace(/\|/g, "\\|") || ""} |`
      );
    }
    lines.push("");
  }
...
async function renderPng(nodeId, scale = 2) {
  const data = await api(
    `/v1/images/${FILE_KEY}?ids=${encodeURIComponent(nodeId)}&format=png&scale=${scale}`
  );
...
function dualTrackHint(name, page) {
  ...
  if (templateHints.some((h) => name.includes(h)))
    return "結構軌（含模板特徵，可能跨入模板軌；實作時再判定）";
  ...
  if (isSet) {
    ...
    if (props.length === 0) {
      ...
      for (const p of props) {
        lines.push(
          `| ${p.name} | \`${p.default}\` | ${p.options.map((o) => `\`${o}\``).join(", ")} |`
        );
      }
    }
    lines.push("");
...
async function writeComponentsForPages(pageList) {
  ...
  for (const page of pageList) {
    ...
    const standalone = comps.filter(
      (c) => c.containing_frame?.pageName === page && !c.containing_frame?.containingStateGroup
    );
...
async function writeIconsCatalog() {
  ...
  const iconStandalone = comps.filter(
    (c) => c.containing_frame?.pageName === "Icons" && !c.containing_frame?.containingStateGroup
  );
...
if (cmd === "cache") {
  ...
  if (!arg) {
    console.error('usage: components "Page1,Page2"');
    process.exit(1);
  }
